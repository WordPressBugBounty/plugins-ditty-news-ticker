...
/**
 * Return setting keys that can't be set through custom display settings
 *
 * These values are always generated by Ditty. The display scripts render some
 * of them as HTML, so they can't be overwritten from a shortcode, block or
 * ajax request.
 * *
 * @since   3.1.70
 */
function ditty_protected_display_setting_keys() {
	return apply_filters( 'ditty_protected_display_setting_keys', array(
		'id',
		'uniqid',
		'title',
		'status',
...
/**
 * Sanitize custom display settings from a shortcode, block or ajax request
 * *
 * @since   3.1.70
 */
function ditty_sanitize_custom_display_settings( $settings, $depth = 0 ) {
	if ( ! is_array( $settings ) || $depth > 5 ) {
		return array();
	}

	$protected_keys = array_map( 'strtolower', ditty_protected_display_setting_keys() );
	...
	foreach ( $settings as $key => $value ) {
		if ( is_string( $key ) ) {
			...
			if ( 0 === $depth && in_array( strtolower( $key ), $protected_keys, true ) ) {
				continue;
			}
		} elseif ( ! is_int( $key ) ) {
			continue;
		...
		if ( is_array( $value ) ) {
			$sanitized_settings[$key] = ditty_sanitize_custom_display_settings( $value, $depth + 1 );
